...
let canvas = document.getElementById("fake-visualizer");
...
function vizBar() {
  ...
  this.growth = 1;
  this.draw = function () {
    c.fillRect(this.startX, this.startY, this.width, -this.height);
  };
...
let bars = [];
for (let i = 0, len = cw / barWidth; i < len; i++) {
  let deviation = Math.abs(len - i - (i + 1));
  let height = ch - Math.pow(deviation, 2);
  let bar = new vizBar();
  bar.maxHeight = height;
  bar.startX = i * barWidth + 2;
  bar.startY = ch;
  bar.width = barWidth - 2;
  bar.growth = bar.maxHeight / 10;
  bars.push(bar);
}
...
function animate() {
  ...
  animateReq = window.requestAnimationFrame(animate);
}
// animate();
